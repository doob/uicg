...
export function checkPackageExists(packageName) {
  ...
  try {
    ...
    const existsInDependencies = packageObj.dependencies && packageObj.dependencies[packageName];
    const existsInDevDependencies = packageObj.devDependencies && packageObj.devDependencies[packageName];
...
export async function checkConfig() {
  ...
  if (!fs.existsSync(configPath)) {
    console.log(error(`🚨 Missing ui-config.json. Run ${success("npx ui-gen init")} to create one.`));
...
export function getJsConfig() {
  ...
  return fs.existsSync(jsconfigPath) ? JSON.parse(fs.readFileSync(jsconfigPath, "utf8")) : false;
}
